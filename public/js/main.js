...
(function ($) {
    ...
    try {
      ...
      char_come.forEach((char_come) => {
        try {
          ...
          console.warn('Title animation error:', e);
        }
      });
    } catch (e) {
      console.warn('Title animations setup error:', e);
    }
  }
    ...
    try {
      ...
      animateTextFromBottom.forEach((item) => {
        try {
          ...
          console.warn('Sub animation error:', e);
        }
      });
    } catch (e) {
      console.warn('Sub animations setup error:', e);
    }
  }
          ...
          tl.from(animateLine3dSplitted.lines, {
            ...
            onComplete: () => {
              try {
                ...
                console.warn('3D animation revert error:', e);
              }
            },
          });
        } catch (e) {
          console.warn('3D animation error:', e);
        }
      });
    } catch (e) {
      console.warn('3D animations setup error:', e);
    }
  }
          ...
          tl.from(animateTextFromBottomSplitted.words, {
            ...
            onComplete: () => {
              try {
                ...
                console.warn('Button text animation revert error:', e);
              }
            },
          });
        } catch (e) {
          console.warn('Button text animation error:', e);
        }
      });
    } catch (e) {
      console.warn('Button text animations setup error:', e);
    }
  }
  ...
  // Parallax image effect - DISABLED to avoid React DOM conflicts
  // The parallax wrapper manipulation conflicts with React's virtual DOM
  // Consider using CSS transforms or a React-compatible library instead
  
  /*
  const imageParallax = document.querySelectorAll(".parallax-image:not([data-react])");

  if (imageParallax.length > 0) {
    // ... parallax code commented out due to React conflicts
  }
  */
